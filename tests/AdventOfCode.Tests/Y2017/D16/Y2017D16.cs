...
namespace AdventOfCode.Tests.Y2017.D16;

[ChallengeName("Permutation Promenade")]
public class Y2017D16
{
    private readonly string _input = File.ReadAllText(@"Y2017\D16\Y2017D16-input.txt", Encoding.UTF8);
    ...
    private int Mod(Func<string, string> step, string startState)
    {
        var state = startState;
        for (var i = 0;; i++)
        {
    ...
    private Func<List<char>, List<char>> ParseMove(string stm, string pattern, Func<string[], Func<List<char>, List<char>>> a)
    {
