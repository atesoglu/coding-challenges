...
namespace AdventOfCode.Tests.Y2023.D24;

[ChallengeName("Never Tell Me The Odds")]
public class Y2023D24
{
    private readonly string _input = File.ReadAllText(@"Y2023\D24\Y2023D24-input.txt", Encoding.UTF8);

    [Fact]
    public void PartOne()
    {
        var particles = Project(ParseParticles(_input), v => (v.x0, v.x1));
    ...
    Vec2 Solve2D(Particle2[] particles)
    {
        ...
        var translateV = (Particle2 p, Vec2 vel) => new Particle2(p.pos, new Vec2(p.vel.x0 - vel.x0, p.vel.x1 - vel.x1));
