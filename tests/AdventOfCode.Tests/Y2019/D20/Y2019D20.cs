...
{
    private readonly string _input = File.ReadAllText(@"Y2019\D20\Y2019D20-input.txt", Encoding.UTF8);
        ...
        for (var irow = 0; irow < crow - 1; irow++)
        {
            for (var icol = 0; icol < ccol - 1; icol++)
            {
                foreach (var (drow, dcol) in new[] { (0, 1), (1, 0) })
                {
                    ...
                    {
                        var portal = irow - drow >= 0 && icol - dcol >= 0 && mx[irow - drow][icol - dcol] == '.' ? new Pos2(irow - drow, icol - dcol) : new Pos2(irow + 2 * drow, icol + 2 * dcol);
