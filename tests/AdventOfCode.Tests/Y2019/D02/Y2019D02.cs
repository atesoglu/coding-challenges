...
public class Y2019D02
{
    ...
    public void PartOne()
    {
        var output = ExecIntCode(new IntCodeMachine(_input), 12, 2);

        output.Should().Be(2842648);
    }

    [Fact]
    public void PartTwo()
    {
        var output = ExecIntCode2();

        output.Should().Be(9074);
    }

    private int ExecIntCode2()
    {
        ...
        while (true)
        {
            ...
            switch (opcode)
            {
                ...
                case Opcode.In:
                {
                    if (this.input.Any())
                    {
                        memory[addr(1)] = this.input.Dequeue();
                        ip += 2;
                    }
        ...
        for (var i = 0; i < count && ip < memory.Length; i++)
        {
            try
            {
                ...
                switch (GetOpcode(ip))
                {
                    ...
                    case Opcode.In:
                        sb.AppendLine($"{addr(1)} = input;");
                        ip += 2;
                        break;
                    case Opcode.Out:
                    {
                        sb.AppendLine($"output {arg(1)};");
                        ip += 2;
                        break;
                    }
                    case Opcode.Jnz:
