...
namespace AdventOfCode.Tests.Y2016.D22;

[ChallengeName("Grid Computing")]
public class Y2016D22
{
    private readonly string _input = File.ReadAllText(@"Y2016\D22\Y2016D22-input.txt", Encoding.UTF8);

    [Fact]
    public void PartOne()
    {
        var nodes = Parse(_input);
        var r = 0;
    ...
    private class Grid
    {
        ...
        private int crow => nodes.GetLength(0);

        public int ccol => nodes.GetLength(1);
        ...
        public bool Wall(int irow, int icol) => nodes[irow, icol].used > nodes[irowEmpty, icolEmpty].size;
