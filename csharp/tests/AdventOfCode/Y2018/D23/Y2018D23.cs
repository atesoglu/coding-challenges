...
namespace AdventOfCode.Y2018.D23;
...
public class Y2018D23
{
    ...
    public void PartOne()
    {
        var drones = Parse(_input);
        var maxRange = drones.Select(drone => drone.r).Max();
        var maxDrone = drones.Single(drone => drone.r == maxRange);
        var output = drones.Count(drone => Dist(drone.pos, maxDrone.pos) <= maxRange);

        output.Should().Be(609);
    }
    ...
    public void PartTwo()
    {
        ...
        var output = Solve(new Box((minX, minY, minZ), (maxX - minX + 1, maxY - minY + 1, maxZ - minZ + 1)), drones).pt;

        output.Should().Be(130370534);
    }
