...
class Program
{
    static int Main(string[] args)
    {
        if (args.Length < 2)
        {
            Console.WriteLine("Usage: dotnet run --project csharp/src/HackerRank -- --problem <ProblemName>");
            Console.WriteLine("Example: dotnet run --project csharp/src/HackerRank -- --problem SolveMe");
            return 1;
        }
        ...
        try
        {
            var className = $"HackerRank.Solutions.{problemName}";
